...
```{r, warning = FALSE}
# nbr raster list
fires.nbr <- list(nbr_BC, nbr_HR, nbr_School, nbr_Egley, nbr_Easy, nbr_Flagtail, nbr_Calamity, nbr_Wheeler)
...
for(i in 1:length(fires)){
  names(fires.nbr[[i]]) <- as.character(c(1984:2021))
}

#extract nbr from rasters
for(i in 1:length(fires)){
  vals <- as.data.frame(extract(fires.nbr[[i]], fires.field.pts[[i]])) %>% 
    mutate(fire = fires.abbrev[i])
  coords <- as.data.frame(fires.field.pts[[i]]) # add x and y coordinates to nbr values dataframes
...
for(i in 1:length(fires)){
  vals <- as.data.frame(extract(fires.nbr[[i]], fires.rand.pts[[i]])) %>% 
    mutate(fire = fires.abbrev[i])
  coords <- as.data.frame(fires.rand.pts[[i]]) # add x and y coordinates to nbr values dataframes
  vals <- cbind(coords, vals)
...
vals_all_fires <- rbind(vals_BC, vals_HR, vals_School, vals_Egley, vals_Calamity,
                        vals_Flagtail, vals_Easy, vals_Wheeler) %>% 
  select(-Long.1, -Lat.1)
```
...
long.nbr.df <- vals_enhanced_all_fires %>% 
  unite('x_y', c(Long, Lat), sep = '_', remove = F) %>% 
  pivot_longer(cols = !c(x_y, Long, Lat, Plot, fire, hli, canopy_cover, veg_height, cwd_present, cwd_future, TE_present, TE_future, elevation), names_to = 'year', values_to = 'nbr') %>% 
...
# now, extracting data from rasters from appropriate years and saving into a dataframe  
  ...
  coords <- as.data.frame(pts) # add x and y coordinates to nbr values dataframes
  vals <- cbind(coords, vals) %>% 
    select(-Long.1, -Lat.1) %>% 
    mutate(year = yrs[j])
...
wide.df <- wide.df %>% 
  mutate(dnbr = `9`-`10`) %>% 
  filter(dnbr > ifelse(is.na(Plot), 0.66*predisturbance_nbr, -9999)) # Checked field data already, so not running this test on field data
```
...
```{r}
long.df <- wide.df %>% 
  pivot_longer(cols = !c(x_y, Long, Lat, Plot, hli, canopy_cover, veg_height, fire, predisturbance_nbr, postdisturbance_nbr, disturbance_nbr, delta_nbr, dnbr, cwd_present, cwd_future, TE_present, TE_future, elevation), names_to = 'relative_year', values_to = 'nbr') %>% 
...
prism.merge.df <- merge(long.nbr.df, prism_df_yearly, by = c('x_y', 'year')) %>% 
...
# Saving Datasets
```{r}
write.csv(wide.df, file = here('data', 'processed_data', 'wide.df.csv'))
write.csv(long.df.prism, file = here('data', 'processed_data', 'long.df.csv'))
write.csv(field.data.both, file = here('data', 'processed_data', 'field.data.both.csv'))
```
